...
- 複数アルゴリズムを同一条件で実行し、実行ごとの結果を `runs/` に保存する。
...
# → runs/YYYYMMDD_HHMMSS_homogeneous_small_sanity_K5_M2_T50000/summary.csv
# → runs/YYYYMMDD_HHMMSS_homogeneous_small_sanity_K5_M2_T50000/curves.csv
# → runs/YYYYMMDD_HHMMSS_homogeneous_small_sanity_K5_M2_T50000/*.png
# → runs/YYYYMMDD_HHMMSS_homogeneous_small_sanity_K5_M2_T50000/run_config.json
```
...
- `--output-root`: run ディレクトリを作る親ディレクトリ。デフォルトは `runs`。
...
| ディレクトリ | 用途 |
|------------|------|
| `runs/YYYYMMDD_HHMMSS_<experiment>/summary.csv` | trial 単位の集計 |
| `runs/YYYYMMDD_HHMMSS_<experiment>/curves.csv` | regret curve 用の時系列 |
| `runs/YYYYMMDD_HHMMSS_<experiment>/*.png` | グラフ |
| `runs/YYYYMMDD_HHMMSS_<experiment>/run_config.json` | 実行設定 |
