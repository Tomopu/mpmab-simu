...
作成日: 2026-05-22  
更新日: 2026-06-11（Heterogeneous セクション追加）
...
1. [前提条件](#前提条件)
2. [Homogeneous 比較実験（compare_homogeneous.py）](#homogeneous-比較実験)
3. [Heterogeneous 比較実験（compare_heterogeneous.py）](#heterogeneous-比較実験)
4. [テスト実行](#テスト実行)
...
| オプション | デフォルト | 内容 |
|---|---|---|
| `--experiment` | `small` | 実験プリセット名。`small` / `asym` から選ぶ。 |
| `--trials N` | プリセット依存 | trial 数。 |
| `--horizon T` | プリセット依存 | horizon T。 |
| `--K K` | プリセット依存 | arm 数 K を上書き。`--means-matrix` と合わせて使う。 |
| `--M M` | プリセット依存 | player 数 M を上書き。`--means-matrix` と合わせて使う。 |
...
| `total_steps` | 使用した実ステップ数 |
| `init_duration` | 初期化フェーズ全体のステップ数 |
| `ortho_duration` | Orthogonalization / FindMultipleGoodArms のステップ数 |
| `rank_assignment_duration` | Rank Assignment / ParallelVMC+VNP のステップ数 |
| `init_sample_duration` | Initial Sampling のステップ数 |
